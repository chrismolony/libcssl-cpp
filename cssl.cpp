...
const char *my_cssl::cssl_geterrormsg()
{
...
/* opens the port */
cssl_t *my_cssl::cssl_open(const char *fname,
		  ...
		  int stop)
{
	...
	serial = (cssl_t *)calloc(1,sizeof(__cssl_t));//calloc返回的是void*这里需要强制类型转换C++与C的不同
    /* oops, no memory */
    if (!serial) {
...
void my_cssl::cssl_close(cssl_t *serial)
{
    cssl_t *cur;
...
void my_cssl::cssl_setup(cssl_t *serial,
		   int baud,
		   int bits,
		   int parity,
		   int stop)
{
    ...
    //modify StevenShi @2017/03/02
	if(parity)
	    serial->tio.c_iflag |= INPCK;
	else
		serial->tio.c_iflag=IGNPAR;

	serial->tio.c_oflag &= ~OPOST;//output
    //serial->tio.c_lflag=0;
	serial->tio.c_lflag &= ~(ICANON | ECHO | ECHOE | ISIG);//input原始模式
    //serial->tio.c_cc[VMIN]=1;
	/* wait for 1 characters to come in before read returns */	
	serial->tio.c_cc[VMIN] = 1;//该值影响接收数据时的中断次数
								// 如果发送报文长度大于1个字符则会发生接收不全的现象
    serial->tio.c_cc[VTIME]=1;//两个字节之间的时间差单位百毫秒
...
void my_cssl::cssl_setflowcontrol(cssl_t *serial,
			 int rtscts,
			 int xonxoff)
{
    if (!cssl_started) {
...
void my_cssl::cssl_settimeout(cssl_t *serial, int timeout)
{
    if (!cssl_started) {
	cssl_error=CSSL_ERROR_NOTSTARTED;
	return;
...
/* sending a char */
void my_cssl::cssl_putchar(cssl_t *serial,
		     char c)
{
    if (!cssl_started) {
	cssl_error=CSSL_ERROR_NOTSTARTED;
...
void my_cssl::cssl_putstring(cssl_t *serial,
		     char *str)
{
    if (!cssl_started) {
	cssl_error=CSSL_ERROR_NOTSTARTED;
...
void my_cssl::cssl_putdata(cssl_t *serial,
		  uint8_t *data,
		  int datalen)
{
    if (!cssl_started) {
...
void my_cssl::cssl_drain(cssl_t *serial)
{
    if (!cssl_started) {
	cssl_error=CSSL_ERROR_NOTSTARTED;
	return;
...
int my_cssl::cssl_getchar(cssl_t *serial)
{
    int result;
    uint8_t c;
...
int my_cssl::cssl_getdata(cssl_t *serial,
		 uint8_t *buffer,
		 int size)
{
...
void my_cssl::cssl_handler(int signo, siginfo_t *info, void *ignored)
{
    cssl_t *cur;
    int n;
	//printf("handler\n");
